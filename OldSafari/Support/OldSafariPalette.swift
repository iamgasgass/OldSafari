...
/// Centralized color palette that reproduces the exact look of the
/// OldOS 2.0.8 Safari chrome (brushed blue-grey metal + gloss address
/// field). Keeping every color in one place means the UI stays
/// pixel-consistent across the address bar, toolbar, tabs and sheets.
enum OldSafariPalette {

    // Chrome gradients (top address bar strip + bottom toolbar).
    ...
    // Progress bar (classic Safari loading indicator).
